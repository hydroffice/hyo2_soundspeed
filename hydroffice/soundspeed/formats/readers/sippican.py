...
class Sippican(AbstractTextReader):
    ...
    def read(self, data_path, settings, callbacks=CliCallbacks(), progress=None):
        ...
        ftype = data_path[-3:].lower()
        if ftype == 'bin':
            pass
            #self._parse_header_bin()
            #self._parse_body_bin()
        elif ftype == 'txt':
            self._parse_header_txt()
            self._parse_body_txt()
        else:
            self._parse_header_edf()
            self._parse_body_edf()
        ...
        for line in self.lines:
            ...
            if line.startswith(self.tk_latitude):  # latitude
                ...
                try:
                    ...
                    self.ssp.cur.meta.sensor_type = Dicts.sensor_types['Unknown']
                    logger.warning('reverted to unknown probe type at line #%s' % self.samples_offset)
            elif line.startswith('Depth'): # field headers
                col = 0  # field column           
    ...
    def _parse_body_txt(self):
        ...
        for line in self.lines[self.samples_offset:len(self.lines)]:
            ...
            # sound speed
            try:
                #self.ssp.cur.data.speed[count] = 1500
