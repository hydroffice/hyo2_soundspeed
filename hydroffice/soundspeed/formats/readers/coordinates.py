...
def GetCoordFromUser(text_lat="Enter Latitude", text_lon="Enter Longitude", title_lat="Latitude", title_lon="Longitude", def_lat='', def_lon='', parent=None, assume_W=False, assume_S=False):
    import wx
    latstr="fail"
    lat = None
    formats = "\n".join([constructor.supported_formats for constructor in (DecDegreesCoordinate, DMCoordinate, DMSCoordinate)])
    text_lat = text_lat+'\n\n'+formats
    while latstr and lat==None: #bail out when we get a legit lat value or user cancels (returns "" for latstr) 
        latstr = wx.GetTextFromUser(("" if latstr=="fail" else "Retry -- use formats below\n\n")+text_lat, title_lat, (def_lat if latstr=="fail" else latstr), parent=None)
...
class BaseCoordinate:
  ...
  def __init__(self, lat, lon):
    if isinstance(lat, (int, float)) and isinstance(lon, (int, float)): # pass non-string value straight through; also, avoid replacing 'e' in case of (lon) scientific notation formatting
        self.lat,self.lon = lat,lon
    else:
        self.lat, self.lon = map(float, (str(lat).lower().replace('n', '').replace('s', '').replace('w', '').replace('e', ''),
...
class DMCoordinate(BaseCoordinate):
  ...
  def __init__(self, lat, lon):
    for p in (r"^(\d{2,3})(\d{2}(\.\d+)?)\s*\'?,?\s*([NSEWnsew])\s*$", #check for 6032.0620000,N and 14656.8280000,W
