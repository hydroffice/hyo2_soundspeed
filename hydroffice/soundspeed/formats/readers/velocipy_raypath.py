...
class Raypath:
  ...
  def __init__(self, data):
    '''data should be an n by 3 numpy.array (travel time, depth, across
    track)
    '''
    ...
    if data.shape[1] != 3:
      raise TypeError("A ray path must be a list of 3-tuples... the shape of this array is '%s' when it should be '([len], 3)'" % str(data.shape))
  ...
  def __add__(self, other):
    ...
    assert (a[:minlength, 0] == b[:minlength, 0]).all() # == returns a list of bools and .all() asks whether they are "all" True
