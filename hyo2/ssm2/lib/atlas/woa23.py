...
class Woa23(AbstractAtlas):
    """WOA23 atlas"""

    def __init__(self, data_folder: str, prj: 'SoundSpeedLibrary') -> None:
        super(Woa23, self).__init__(data_folder=data_folder, prj=prj)
        self.name = self.__class__.__name__
        self.desc = "World Ocean Atlas 2023"
    ...
    def is_present(self) -> bool:
        ...
        check_woa23_temp = os.path.join(self.data_folder, 'temp', 'woa23_decav_t00_04.nc')
        check_woa23_sal = os.path.join(self.data_folder, 'sal', 'woa23_decav_s01_04.nc')  # s00 is not required
        if os.path.exists(check_woa23_temp) and os.path.exists(check_woa23_sal):
            return True
        # logger.info('unable to locate the WOA23 db at the default location: %s' % self.folder)

        # no way to find the database
        logger.warning("unable to locate the WOA23 db")
        return False

    def download_db(self) -> bool:
        """try to download the data set"""
        logger.debug('downloading WOA23 atlas')

        try:
            gd = GoogleDrive(show_progress=True, debug_mode=True, progress=self.prj.progress)
            data_zip_src = "https://drive.google.com/file/d/1ie1_hNH4EbAJ51mReLj_a6KJmZUimR1H/view?usp=sharing"
            ...
            gd = GoogleDrive(show_progress=True, debug_mode=True, progress=self.prj.progress)
            data_zip_src = "https://drive.google.com/file/d/1n38NwV7oUjoaGtDtitJHkgTxh47sFUCo/view?usp=sharing"
            ...
            logger.error('during WOA23 download and unzip: %s' % e)
            return False
    ...
    def load_grids(self) -> bool:
        ...
        try:
            for i in range(1, 17):
                t_path = os.path.join(self.data_folder, "temp", "woa23_decav_t%02d_04.nc" % i)
                self.t.append(Dataset(t_path))
            for i in range(1, 17):
                s_path = os.path.join(self.data_folder, "sal", "woa23_decav_s%02d_04.nc" % i)
    ...
    def query(self, lat: float, lon: float, dtstamp: Union[dt, None] = None, server_mode: bool = False):
        ...
        ssp.meta.probe_type = Dicts.probe_types['WOA23']
        ...
        ssp_min.meta.probe_type = Dicts.probe_types['WOA23']
    ...
    def __repr__(self) -> str:
        msg = "%s" % super(Woa23, self).__repr__()
        return msg
