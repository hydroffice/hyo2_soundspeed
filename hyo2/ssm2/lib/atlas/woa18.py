...
class Woa18(AbstractAtlas):
    ...
    def __init__(self, data_folder: str, prj: 'SoundSpeedLibrary') -> None:
        ...
        self.desc = "World Ocean Atlas 2018"
    ...
    def is_present(self) -> bool:
        ...
        check_woa18_sal = os.path.join(self.data_folder, 'sal', 'woa18_decav_s01_04.nc')  # s00 is not required
        if os.path.exists(check_woa18_temp) and os.path.exists(check_woa18_sal):
            return True
        # logger.info('unable to locate the WOA18 db at the default location: %s' % self.folder)

        # no way to find the database
        logger.warning("unable to locate the WOA18 db")
        return False
    ...
    def download_db(self) -> bool:
        ...
        try:  # TODO
            ...
            logger.error('during WOA18 download and unzip: %s' % e)
            return False
    ...
    def load_grids(self) -> bool:
        ...
        try:
            ...
            for i in range(1, 17):
                s_path = os.path.join(self.data_folder, "sal", "woa18_decav_s%02d_04.nc" % i)
    ...
    def __repr__(self) -> str:
        msg = "%s" % super(Woa18, self).__repr__()
        return msg
