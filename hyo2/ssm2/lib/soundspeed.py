...
class SoundSpeedLibrary:
    ...
    @property
    def name_readers(self) -> list[str]:
        return formats.name_readers

    @property
    def ext_readers(self) -> list[set[str]]:
        return formats.ext_readers

    @property
    def desc_readers(self) -> list[str]:
    ...
    @property
    def name_writers(self) -> list[str]:
        return formats.name_writers

    @property
    def ext_writers(self) -> list[set[str]]:
        return formats.ext_writers

    @property
    def desc_writers(self) -> list[str]:
    ...
    def retrieve_woa18(self) -> None:
        ...
        self.ssp = self.atlases.woa18.query(lat=lat, lon=lon, datestamp=utc_time)
    ...
    def retrieve_woa23(self) -> None:
        ...
        self.ssp = self.atlases.woa23.query(lat=lat, lon=lon, datestamp=utc_time)
    ...
    def retrieve_ngofs2(self) -> None:
        ...
        self.ssp = self.atlases.ngofs2.query(lat=lat, lon=lon, datestamp=utc_time)
    ...
    def retrieve_sscofs(self) -> None:
        ...
        self.ssp = self.atlases.sscofs.query(lat=lat, lon=lon, datestamp=utc_time)
    ...
    def retrieve_lmhofs(self) -> None:
        ...
        self.ssp = self.atlases.lmhofs.query(lat=lat, lon=lon, datestamp=utc_time)
    ...
    def retrieve_wcofs(self) -> None:
        ...
        self.ssp = self.atlases.wcofs.query(lat=lat, lon=lon, datestamp=utc_time)
        ...
        for i, name in enumerate(data_formats):
            ...
            if not has_data_files:  # we don't have the output file names
                ...
                if data_files is None:
                    raise RuntimeError("data_files not set")
