...
from hyo.soundspeedmanager.dialogs.seacat import add_btn, get_last_comport, AutoSeacat, SelectCastsDlg, \
...
class SeacatDialog(AbstractDialog):
            ...
            with AutoSeacat(progbar=self.progress) as cat:  # with auto-closes port even if an exception is thrown
                if cat.isOpen():
                    ...
                    if headers:
                        ...
                        # noinspection PyCallByClass
                        QtGui.QMessageBox.information(self, "Notice", "No casts in the device")
                        self.reject()
        ...
        if paths:
            ...
            if is_ok:
                ...
                if seabird_datacnv:
                    ...
                    if failed:
                        # noinspection PyCallByClass
                        QtGui.QMessageBox.information(self, "Error in Conversion",
                                                      "The following casts failed to convert:\n\n" + "\n".join(failed))
                else:
                    logger.info('Seabird DataCNV.exe not found\n')
            else:
                # noinspection PyCallByClass
                QtGui.QMessageBox.information(self, confile_path_or_messages[0], confile_path_or_messages[1])
        else:
    ...
    def on_status(self):
        ...
        with AutoSeacat(progbar=self.progress) as cat:  # with auto-closes port even if an exception is thrown
            ...
            logger.info(msg)
            # noinspection PyCallByClass
            QtGui.QMessageBox.information(self, "SBE SeaCAT", msg)
        ...
        with AutoSeacat(progbar=self.progress) as cat:  # with auto-closes port even if an exception is thrown
            if cat.isOpen():
                ...
                try:
                    ...
                    # noinspection PyCallByClass
                    ret = QtGui.QMessageBox.question(self, "Confirm Seacat Time Change", msg,
        ...
        with AutoSeacat(progbar=self.progress) as cat:  # with auto-closes port even if an exception is thrown
            if cat.isOpen():
                try:
                    ...
                    if diff > datetime.timedelta(0, 3 * 60) or diff < datetime.timedelta(0, -3 * 60):
                        msg = """The Seacat time is %s
                            ...
                            Do you want to reset the Seacat time when initializing?""" % (dt.isoformat(),
                                                                                          dt_now.isoformat())
                        # noinspection PyCallByClass
                        ret = QtGui.QMessageBox.question(self, "Time Mismatch", msg,
                                                         QtGui.QMessageBox.Yes | QtGui.QMessageBox.No)
                        if ret == QtGui.QMessageBox.Yes:
                            bSetTime = True
                    ...
                    # noinspection PyCallByClass
                    ret = QtGui.QMessageBox.question(self, "Confirm Seacat Init",
                ...
                if not conname:
                    ...
                    if conname:
                        ...
                        if not matches_convention:
                            ...
                            # noinspection PyCallByClass
                            ret = QtGui.QMessageBox.question(self, "Copy and Rename?", msg,
                                                             QtGui.QMessageBox.Yes | QtGui.QMessageBox.No)
                            if ret == QtGui.QMessageBox.Yes:
