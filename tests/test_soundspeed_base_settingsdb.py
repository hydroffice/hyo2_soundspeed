...
class TestSoundSpeedSettingsDb(unittest.TestCase):
    ...
    def test_create_settings_db_and_check_db_path(self):
        db = SettingsDb(data_folder=self.data_folder, db_file=self.db_name)
    ...
    def test_compare_library_version(self):
        from hydroffice.soundspeed import __version__ as lib_version
        db = SettingsDb(data_folder=self.data_folder, db_file=self.db_name)
        self.assertEqual(db.library_version, lib_version)
        db.close()
...
def suite():
    ...
    s.addTests(unittest.TestLoader().loadTestsFromTestCase(TestSoundSpeedSettingsDb))
    return s
